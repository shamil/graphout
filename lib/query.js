...
var Query = module.exports = function(app, options) {
    ...
    if (app.options.splay) {
        // get random number between 1000 and query interval
        delay = 1000 + parseInt(Math.random() * app.options.interval);
        this.logger.debug("delaying initial query by", delay, "milliseconds", {query: this.options.name});
    }
