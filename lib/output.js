...
Output.prototype.onRaw = function(data, query_options) {
    ...
    if (!this.filter.test(query_options.name)) {
        this.log.trace('skipping query, the filter didn\'t match', {query_name: query_options.name, event: 'raw'});
        return;
    }
...
Output.prototype.onValues = function(values, query_options) {
    // do not proceed if filter was not matched
    if (!this.filter.test(query_options.name)) {
        this.log.trace('skipping query, the filter didn\'t match', {query_name: query_options.name, event: 'values'});
        return;
    }
